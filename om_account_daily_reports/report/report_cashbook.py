...
class ReportCashBook(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        # # Step 1: Search for relevant payments with 'cash' journal
        # payments = self.env['account.payment'].search([
        #     ('payment_expense', '=', True),
        #     ('state', 'in', ['paid', 'in_process','expense_done']),
        #     ('journal_id.type', '=', 'cash')
        # ])
        # print('payments',payments)
        #
        # # Step 2: Get all unique expense_account_id from those payments
        # expense_accounts = payments.mapped('expense_account_id')
        #
        # print("expense_accounts",expense_accounts)
        #
        # # Step 3: Combine with existing accounts and remove duplicates
        # accounts |= expense_accounts  # Union with existing accounts
        accounts = self.env['account.account'].browse(set(accounts.ids))  # Ensure uniqueness
        # # accounts = accounts.sorted(key=lambda a: a.id)  # Optional: sort if needed
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
                        ...
                        JOIN account_journal j ON (l.journal_id=j.id)\
                        JOIN account_account acc ON (l.account_id = acc.id) \
                        WHERE l.account_id IN %s  AND j.type = 'cash' ''' + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
...
#         #add condition start
#
#         accounts = self.env['account.account'].browse(set(accounts.ids))
#
#         # add condition end
#
#
