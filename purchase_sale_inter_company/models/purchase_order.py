...
class PurchaseOrder(models.Model):
    ...
    def button_approve(self, force=False):
        ...
        if not self.env.context.get('po_from_so'):
            ...
            for order in self.filtered("auto_sale_order_id"):
                for line in order.order_line.sudo():
                    if line.auto_sale_line_id:
                        ...
                        line.product_uom_qty = line.auto_sale_line_id.product_uom_qty
        return res
    ...
    def _inter_company_create_sale_order(self, dest_company):
        ...
        if dest_company.sale_auto_validation:
            context = {'so_from_po': True}
            sale_order.with_user(intercompany_user.id).with_context(context).sudo().action_confirm()
