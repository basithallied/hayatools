...
class InsTrialBalance(models.TransientModel):
    ...
    def prepare_where(self, mode='strict'):
        ...
        cmpny_ids = self.env.company.ids + self.env.companies.ids
        ...
        # Add Journal restriction for users
        user = self.env.user
        if hasattr(user, 'journal_ids'): # if res.users has property journal_ids
            journals = user.journal_ids
            if journals:
                where += 'AND j.id IN %s ' % str(tuple(journals.ids) + tuple([0]))
    ...
    def prepare_main_lines(self):
        ...
        account_company_domain = [('company_ids', 'in', self.env.company.ids + self.env.company.parent_ids.ids)]
        ...
        total = {
            ...
            'debit': sum([a.get('debit', 0) or 0 for a in gl_lines]) or 0,
            'credit': sum([a.get('credit', 0) or 0 for a in gl_lines]) or 0,
            'balance': sum([a.get('balance', 0) or 0 for a in gl_lines]) or 0,
            'comparison_debit': sum([a.get('comparison_debit', 0) or 0 for a in gl_lines]) or 0,
            'comparison_credit': sum([a.get('comparison_credit', 0) or 0 for a in gl_lines]) or 0,
            'comparison_balance': sum([a.get('comparison_balance', 0) or 0 for a in gl_lines]) or 0,
            'initial_debit': sum([a.get('initial_debit', 0) or 0 for a in gl_lines]) or 0,
            'initial_credit': sum([a.get('initial_credit', 0) or 0 for a in gl_lines]) or 0,
            'initial_balance': sum([a.get('initial_balance', 0) or 0 for a in gl_lines]) or 0,
            'ending_debit': sum([a.get('ending_debit', 0) or 0 for a in gl_lines]) or 0,
            'ending_credit': sum([a.get('ending_credit', 0) or 0 for a in gl_lines]) or 0,
            'ending_balance': sum([a.get('ending_balance', 0) or 0 for a in gl_lines]) or 0,
        }
        gl_lines.append(total)
        return gl_lines
