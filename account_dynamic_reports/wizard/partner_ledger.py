...
class InsPartnerLedger(models.TransientModel):
    ...
    def _partner_sub_group_domain(self):
        sub_groups = self.env['res.partner'].search([]).mapped('vendor_sub_group_id')
    ...
    partner_sub_group_ids = fields.Many2many('res.partner', 'partner_ledger_subgroup_rel', 'ledger_id',
        'subgroup_id', string='Vendor Sub Group', domain=lambda self: self._partner_sub_group_domain()
    )
    ...
    def prepare_main_lines(self):
        ...
        if self.partner_sub_group_ids:
            partner_company_domain += ['|', ('id', 'in', self.partner_sub_group_ids.ids), ('vendor_sub_group_id', 'in', self.partner_sub_group_ids.ids)]
            ...
            sql = ('''
                SELECT
                    COALESCE(
                        SUM(
                            CASE
                                WHEN l.debit > 0 AND l.amount_currency > 0
                                THEN l.amount_currency
                                ELSE 0
                            END
                        ), 0
                    ) AS debit_currency,  -- Fixed alias and placement
                    COALESCE(
                        SUM(
                            CASE
                                WHEN l.credit > 0 AND l.amount_currency > 0
                                THEN l.amount_currency
                                ELSE 0
                            END
                        ), 0
                    ...
                    COALESCE(SUM(l.amount_currency),0) AS balance_currency,
    ...
    def prepare_detailed_lines(self, move_line_ids=[], partner_id=False):
        ...
        sql = ('''
                SELECT
                    ...
                    a.code_store ->> '1' AS account_code,
                    m.name AS move_name,
                    m.id AS move_id,
                    ...
                    CASE
                        WHEN l.currency_id IS NOT NULL AND l.amount_currency IS NOT NULL
                        THEN ABS(l.amount_currency)
                        ELSE 0
                    END AS credit_currency
    ...
    def prepare_values_for_component(self):
        ...
        return {
            ...
            'defaultPartnerSubGroupValues': [{'value': a.id, 'label': a.name} for a in self.partner_sub_group_ids],
