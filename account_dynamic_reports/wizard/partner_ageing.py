...
class InsPartnerAgeing(models.TransientModel):
        ...
        for partner in partner_ids:
            ...
            for period in period_dict:
                ...
                sql = """
                        ...
                        """ % (as_on_date, as_on_date, type, str(tuple(self.env.companies.ids) + tuple([0])))
                self.env.cr.execute(sql + where)
                fetch_dict = self.env.cr.dictfetchall() or 0.0
                age_dict.update({period_dict[period]['name']: fetch_dict[0].get('balance') or 0})
                age_dict['total'] += fetch_dict[0].get('balance') or 0
    ...
    def prepare_detailed_lines(self, move_line_ids=[], account_id=False):
        ...
        if account_id:
            ...
            # Dynamically add ranges based on periods
            for period, values in period_dict.items():
                start = values.get('start')
                stop = values.get('stop')
                range_alias = f"range_{period}"
                
                if start and stop:
                    select += f"""
                        CASE
                            WHEN COALESCE(l.date_maturity, l.date) BETWEEN '{stop}' AND '{start}'
                            THEN
                                SUM(l.balance) +
                                SUM(COALESCE(
                                    (SELECT SUM(amount) 
                                        FROM account_partial_reconcile 
                                        WHERE credit_move_id = l.id AND max_date <= '{as_on_date}'), 0
                                )) -
                                SUM(COALESCE(
                                    (SELECT SUM(amount) 
                                        FROM account_partial_reconcile 
                                        WHERE debit_move_id = l.id AND max_date <= '{as_on_date}'), 0
                                ))
                            ELSE 0
                        END AS {range_alias},"""
                elif not start:
                    select += f"""
                        CASE
                            WHEN COALESCE(l.date_maturity, l.date) >= '{stop}'
                            THEN
                                SUM(l.balance) +
                                SUM(COALESCE(
                                    (SELECT SUM(amount) 
                    ...
                    select += f"""
                        CASE
                            WHEN COALESCE(l.date_maturity, l.date) <= '{start}'
                            THEN
                                SUM(l.balance) +
                                SUM(COALESCE(
                                    (SELECT SUM(amount) 
            ...
            sql = f"""
                ...
                LEFT JOIN res_currency AS cc ON l.company_currency_id = cc.id
                WHERE l.id IN {tuple(move_line_ids) + (0,)}
                GROUP BY l.date, l.date_maturity, m.id, m.name, j.code, cc.id, l.id, a.code_store
            """
            ...
            move_lines = []
            for m in final_list:
                m['total'] = sum(m.get(f'range_{i}', 0) for i in period_dict)
                if any(m.get(f'range_{i}', 0) for i in period_dict):
                    move_lines.append(m)
