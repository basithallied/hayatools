...
class InsFinancialReport(models.TransientModel):
    ...
    def prepare_where(self):
        where = ' WHERE (1=1) AND l.parent_state = \'posted\' '
        if self.journal_ids:
            where += ' AND j.id IN %s ' % str(tuple(self.journal_ids.ids) + tuple([0]))
        if self.company_id:
            where += ' AND l.company_id IN %s ' % str(tuple(self.env.companies.ids) + tuple([0]))
        if self.account_tag_ids:
            # Add Account Tag restriction
            account_ids = self.env['account.account'].search([('tag_ids', 'in', self.account_tag_ids.ids)])
            if account_ids:
                where += ' AND a.id NOT IN %s ' % str(tuple(account_ids.ids) + tuple([0]))
