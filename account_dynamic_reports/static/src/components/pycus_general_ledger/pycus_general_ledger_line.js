...
export class PycusGeneralLedgerLine extends Component {
    setup(){
        ...
        this.fetchMoveLines = async () => {
            if(this.state.active_ids.length > 0){
                this.state.data = await this.ormService.call(
                    'ins.general.ledger', 'prepare_detailed_lines',
    ...
    formatFieldMonetary(value,currency_id=0) {
        if(!value){
            value = 0
        }
        if(!currency_id){
            currency_id = this.state.account_line.currency_id
        }
...
PycusGeneralLedgerLine.components = { }
PycusGeneralLedgerLine.template = 'account_dynamic_reports.generalLedgerLine';