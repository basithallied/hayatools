...
class EmployeeTransfer(models.Model):
    ...
    transfer_type = fields.Selection(
        [('internal', 'Internal'), ('department', 'Department')],
        string="Transfer Type", required=True,
        help="Select the type of transfer: Internal (between branches) or Departmental (within the same branch).")
    old_employee_id = fields.Many2one(
        'hr.employee', string='Old Employee', help='Old employee details')   
    
    transfer_date = fields.Date(string='Requested Date', default=fields.Date.today(),
                                help="Transfer date")
    ...
    current_department_id = fields.Many2one(
        'hr.department', string='Current Department',
        related='employee_id.department_id', readonly=True,
        help="The employee's current department")
    
    current_job_position_id = fields.Many2one(
        'hr.job', string='Current Job Position',
    ...
    transfered_department_id = fields.Many2one(
        'hr.department', string='Transfered Department',)
        
    transfered_position_id = fields.Many2one(
        'hr.job', string='Transfered Job Position',
        help="The employee's current job position")
    ...
    def action_transfer(self):
        ...
        if self.transfer_type == 'internal':
            ...
            if not self.transfer_company_id:
                raise UserError(_("You should select a company for internal transfer."))
            if self.transfer_company_id == self.company_id:
                raise UserError(_("You cannot transfer an employee to the same company."))
        elif self.transfer_type == 'department':
            # Departmental transfer validation
            if not self.transfered_department_id:
                raise UserError(_("You should select a new department for departmental transfer."))
            if self.transfered_department_id == self.current_department_id:
                raise UserError(_("You cannot transfer an employee to the same department."))

            # Update employee directly for departmental transfers
            self.employee_id.sudo().write({
                'department_id': self.transfered_department_id.id,
                'job_id': self.transfered_position_id.id,
            })

        self.state = 'transfer'

    def action_receive_employee(self):
        """Handle receiving the employee after transfer."""
        if self.transfer_type == 'internal':
            # Internal transfer logic
            self.old_employee_id = self.employee_id
            employee_data = self.employee_id.sudo().read(
                ['name', 'private_email', 'gender', 'identification_id', 'passport_id'])[0]
            del employee_data['id']
            employee_data.update({'company_id': self.transfer_company_id.id})
            
            # Create a new employee in the target company
            new_employee = self.env['hr.employee'].sudo().create(employee_data)
            
            # End active contracts for the old employee (if any exist)
            contracts = self.env['hr.contract'].search([('employee_id', '=', self.employee_id.id), ('date_end', '=', False)])
            if contracts:
                contracts.sudo().write({'date_end': fields.Date.today().strftime(DEFAULT_SERVER_DATE_FORMAT)})

            # Update employee references and deactivate old employee
            self.employee_id = new_employee
            self.old_employee_id.sudo().write({'active': False})
            
        elif self.transfer_type == 'department':
            # Departmental transfer logic
            self.employee_id.sudo().write({
                'department_id': self.transfered_department_id.id,
                'job_id': self.transfered_position_id.id,
            })

        # Redirect to new contract creation form for internal transfers
        if self.transfer_type == 'internal':
            return {
                'name': _('Contract'),
                'view_mode': 'form',
                ...
                'context': {
                    ...
                    'default_department_id':self.transfered_department_id.id,
                    'default_job_id':self.transfered_position_id.id
                },
            }
